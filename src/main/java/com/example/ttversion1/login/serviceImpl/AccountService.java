...
//    public AccountService(DecentralizationRepo decentralizationRepo, UserService userService) {
//        this.decentralizationService = decentralizationService;
//    }
    ...
    public List<AccountDTO> GetAllAdmin() {
        return accountRepo.getAllByRoleAdmin().stream().map(
                account -> {
                    ...
                    accountDTO.setDecentralization(roles);
...
@Override
    ...
    public AccountDTO AccountToDTO(Account account) {
        AccountDTO accountDTO=modelMapper.map(account,AccountDTO.class);
        accountDTO.setMatchingPassword("Không thể hiển thị");
        accountDTO.setPassword("Không thẻ hiển thị");
        Optional<Decentralization> decentralization = decentralizationRepo.findByName("USER");
        Set<Decentralization> role = new HashSet<>();
        role.add(decentralization.get());
        accountDTO.setDecentralization(role);
        Optional<UserDTO> userDTO = userService.findUserByEmail(account.getUser().getEmail());
        accountDTO.setUserDTO(userDTO.get());
        return accountDTO;
    }
