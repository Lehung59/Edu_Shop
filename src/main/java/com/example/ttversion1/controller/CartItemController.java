...
public class CartItemController {
    ...
    @PostMapping(value = "/user/cartItem/insert")
    ResponseEntity<ResponseObject> Insert(
        ...
        Optional<Carts> cart = cartsRepo.getCartsByEmail(email);
        Optional<Product> product = productRepo.findProductByName(productName);
        if(quantity < 0){
            return ResponseEntity.status(HttpStatus.NOT_IMPLEMENTED).body(
                    new ResponseObject("FAILED","Quantity must larger than 0","")
            );
        } else if(cartItem.isPresent()) {
            cartItem.get().setQuantity(cartItem.get().getQuantity() + quantity);
            cartItemRepo.save(cartItem.get());
            return ResponseEntity.status(HttpStatus.OK).body(
                    new ResponseObject("OK", Constants.UPDATE_SUCCESS, cartItem.get())
            );
        } else {
            if (product.isEmpty()) {
                return ResponseEntity.status(HttpStatus.NOT_IMPLEMENTED).body(
...
//        String email = account.getUser().getEmail();
//        Optional<CartItem> cartItem = cartItemRepo.getCartItemByProductnameEmail(productname, email.trim());
//        Optional<Carts> carts = cartsRepo.getCartsByEmail(email.trim());
//        Optional< Product> product = productRepo.findProductByName(productname);
//        if (quantity<0){
//            return ResponseEntity.status(HttpStatus.NOT_IMPLEMENTED).body(
