...
public class OrderDetailController {
    ...
    //Update bảng orderdetail. bang nay
...
//        Optional<Order> order = orderRepo.GetOrderTblByUser(email);
//        if(order.isEmpty()){
//            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(
//                    new ResponseObject("FAILED","Not found order","")
//            );
//        }
//
//
        ...
        if (userDTO.isPresent() && cartItem.isPresent()){
            if(cartItem.isEmpty()){
                return ResponseEntity.status(HttpStatus.NOT_IMPLEMENTED).body(
                        new ResponseObject("FAILED","Không có trong giỏ hàng","")
                );
            } else if (product.get().getStatus()!=1){
                return ResponseEntity.status(HttpStatus.NOT_IMPLEMENTED).body(
...
//    ResponseEntity<ResponseObject> InsertOrderDetail(@ModelAttribute String productName,
//                                                     @ModelAttribute String method){
//        Authentication auth = SecurityContextHolder.getContext().getAuthentication();
//        Account account = accountRepo.findByUsername(auth.getName()).get();
//        String email = account.getUser().getEmail();
//        Optional<CartItem> cartItem = cartItemRepo.getCartItemByProductnameEmail(productName,email);
//        Optional<Product> product = productRepo.findProductIdByName(productName);
//
//        if(cartItem.isEmpty()){
//            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(
...
//            return ResponseEntity.status(HttpStatus.NOT_IMPLEMENTED).body(
//                    new ResponseObject("FAILED","Product no not for sale","")
//            );
//        }
//        if(cartItem.get().getQuantity()>= product.get().getQuantity()){
//            return ResponseEntity.status(HttpStatus.NOT_IMPLEMENTED).body(
//                    new ResponseObject("FAILED","Not enough porduct","")
//            );
//        }
//
//
